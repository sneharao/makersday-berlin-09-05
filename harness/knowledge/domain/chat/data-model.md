...
- **Cross-context tenant guard at chat creation.** `ChatRepo.save(chat)` must reject any new chat whose `libraryId` does not belong to the same `userId`. Enforcement: the chat **application service** (`application/chat/chat.service.ts`) depends on the `library` context's **application service** — `LibraryService.ensureDefaultLibrary(userId)` is invoked to resolve the library this chat will be bound to, and `LibraryService.getArtifactById(userId, artifactId)` is used for citation page-range validation. The repository layer trusts that this assertion has happened upstream. (This honours `harness/knowledge/repo-architecture/dependency-rules.md` "Common Scenarios — I need an application service to call another context's logic → depend on the other context's application service, never reach into another context's repository or gateway." Earlier drafts of this document showed `LibraryRepo` as the dependency; that was corrected in the GR-003 implementation — see `harness/exec-plans/002-gr_003-chat-with-library/`.)

- **v1 library binding.** Chats are auto-bound to the user's auto-provisioned default library at creation time. There is no user-facing library picker. Multi-library UX is a future ticket; the data shape (`Chat.libraryId`) is already correct for that future, so no migration is required.
